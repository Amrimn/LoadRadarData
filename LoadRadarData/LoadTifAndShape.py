...
listFolderTif32, listFolderExtractSlicks=browseRootDir('/home/emna/Documents/Test_Data')
...
for imgFolder in listFolderTif32: # all folder tiff 32bits
    ...
    for image in imageFileList: #
        ...
        crop=gdal.Translate(final+image_name.split('.')[0],image,scaleParams = [[stats[0],stats[1]]],outputType = gdal.GDT_Float32,bandList = [1,1,1]) #GDT_Byte
        ...
        for shapefile in all_shape:
                ...
                #get the shape type is it a seeps or spill?
                isSpill=True
                fill_value=1
                if "spill" not in shapefile:
                    print "IS A SEEP spill"
                elif "seep" not in shapefile:
                    print "IS A SEEP SHAPE"
                    isSpill=False
                    fill_value=2

                # Check if shape is inside the tif
                if isShapeOutsideTif(raster,vector):
        ...
        # creee polygone speill et sieep et inserner dand mm couche
        #gdal.RasterizeLayer(crop, [2], layer spill, burn_values=[1])
        #gdal.RasterizeLayer(crop, [2], layer seeps, burn_values=[1])
        memfeature.SetGeometry(multipolygon_shape)
